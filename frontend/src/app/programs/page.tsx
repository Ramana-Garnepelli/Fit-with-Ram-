...
export default function Programs() {
    ...
    const handleSelectPlan = (planId: string) => {
        ...
        if (!token) {
            router.push(`/login?redirect=${encodeURIComponent(`/pricing?planId=${planId}`)}`);
        } else {
                        ...
                        {programs.map((prog) => (
                            <div key={prog._id} className="group relative overflow-hidden rounded-3xl shadow-xl hover:shadow-2xl transition-all duration-300 bg-white border border-gray-100 flex flex-col">
                                ...
                                <div className="p-8 flex-grow flex flex-col">
                                    ...
                                    <button
                                        onClick={() => handleSelectPlan(prog._id)}
                                        className="block w-full py-4 rounded-xl bg-black text-white font-black hover:bg-gray-800 transition-all text-sm uppercase tracking-widest text-center shadow-lg hover:shadow-black/20 transform hover:-translate-y-1"
                                    >
                                        Select Program
                                    </button>
                                </div>
